...
% Define the range of gray levels for slicing
lower_gray = 100; % Lower threshold
upper_gray = 200; % Upper threshold
...
gray_slice_no_bg(highlighted_region) = gray_image(highlighted_region);  % Retain only the pixels within the range
...
title('UI22EC58');
for bit = 1:8
    bit_plane = bitget(gray_image, bit);  % Extract the bit plane
    subplot(2, 4, bit);
